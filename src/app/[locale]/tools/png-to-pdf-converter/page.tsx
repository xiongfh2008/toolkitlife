...
export default function PngToPdfConverter() {
  const t = useTranslations('tools.png-to-pdf-converter')
  ...
  const handleUpload = useCallback((newFiles: FileList | null) => {
    if (!newFiles) return
    const pngs = Array.from(newFiles).filter(isPng)
  ...
  const convert = async () => {
    ...
    try {
      ...
      for (let i = 0; i < files.length; i++) {
        ...
        const png = await pdf.embedPng(new Uint8Array(bytes))
      ...
      {step === 'upload' && (
        <div className="space-y-4">
          <div
            ...
            onClick={() => document.getElementById('png-in')?.click()}
          ...
          >
            ...
            <input id="png-in" type="file" accept=".png,image/png" multiple className="hidden" onChange={e => handleUpload(e.target.files)} />
          </div>

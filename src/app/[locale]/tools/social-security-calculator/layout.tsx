...
export function generateStaticParams() {
  return [{ slug: "social-security-calculator" }];
}
...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: "tools.social-security-calculator.metadata" });
  return {
    title: t("title"),
    openGraph: {
...
export default async function Layout({
  ...
  setRequestLocale(locale);
  return (
    <ToolMessages slug="social-security-calculator" locale={locale}>
      {children}
    </ToolMessages>
  );
}

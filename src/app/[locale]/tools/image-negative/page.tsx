...
import { negative } from "@/lib/image-effects";

const MAX_DIM = 3000;

export default function ImageNegativePage() {
  const t = useTranslations("tools.image-negative");
  ...
  const process = useCallback(async () => {
    ...
    try {
      ...
      ctx.putImageData(negative(src), 0, 0);
      canvas.toBlob((blob) => {
        if (!blob) return;
      ...
      <div className="max-w-4xl space-y-4">
        {!img ? (
          <div
            ...
            onClick={() => document.getElementById("neg-in")?.click()}
          ...
          <div className="space-y-4">
            <div className="flex flex-wrap items-center gap-3">
              ...
              >
                {processing ? t("status.processing") : t("buttons.invert")}
              </button>
              <button

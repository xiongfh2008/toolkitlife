...
export default function ImageShadowPage() {
  const t = useTranslations("tools.image-shadow");
  ...
  const process = useCallback(async () => {
    ...
    try {
      ...
      const pad = blur * 2 + Math.max(Math.abs(offsetX), Math.abs(offsetY)) + 10;
      canvas.width = w + pad;
      canvas.height = h + pad;
      const ctx = canvas.getContext("2d");
      if (!ctx) return;
      ctx.clearRect(0, 0, canvas.width, canvas.height);
      // Draw shadow: offset image with reduced alpha, then blur it.
      ctx.save();
  ...
  }, [img, processing, blur, offsetX, offsetY, t]);
      ...
      <div className="max-w-4xl space-y-4">
        {!img ? (
          <div
            ...
            onClick={() => document.getElementById("shadow-in")?.click()}
            ...
            {result && (
              <div className="space-y-2">
                ...
                <div className="grid gap-4 sm:grid-cols-2">
                  ...
                  <div className="overflow-hidden rounded-lg border border-zinc-800 bg-zinc-900 p-2">

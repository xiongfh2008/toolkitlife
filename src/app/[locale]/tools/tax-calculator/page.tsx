...
export default function TaxCalculatorPage() {
  ...
  const [mode, setMode] = useState<"flat" | "bracket">("flat");
      ...
      <div className="max-w-3xl space-y-4">
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <div>
            ...
            <select
              value={mode}
              onChange={(e) => setMode(e.target.value as "flat" | "bracket")}
              className="w-full rounded-lg border border-zinc-700 bg-zinc-800 px-4 py-2 text-sm text-zinc-100 outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/40"
            >
              <option value="flat">{t("options.flat")}</option>
              <option value="bracket">{t("options.bracket")}</option>
            </select>
          </div>
          {mode === "flat" && (
            <div>
        ...
        {result && (
          <div className="rounded-lg border border-zinc-800 bg-zinc-900 p-5 space-y-4">
            <div className="flex items-center justify-between">
              ...
              <CopyButton text={result.tax} className="text-xs px-2 py-1" />
            </div>
            <p className="text-3xl font-bold text-blue-400">{result.tax}</p>

...
export function generateStaticParams() {
  return [{ slug: "paycheck-calculator" }];
}
...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: "tools.paycheck-calculator.metadata" });
  return {
    title: t("title"),
    openGraph: {
...
export default async function Layout({
  ...
  setRequestLocale(locale);
  return (
    <ToolMessages slug="paycheck-calculator" locale={locale}>
      {children}
    </ToolMessages>
  );
}

...
import { pixelate } from "@/lib/image-effects";
...
export default function ImagePixelatePage() {
  ...
  const process = useCallback(async () => {
    ...
    try {
      ...
      ctx.putImageData(pixelate(src, blockSize), 0, 0);
      canvas.toBlob((blob) => {
        if (!blob) return;
      ...
      <div className="max-w-4xl space-y-4">
        {!img ? (
          <div
            ...
            onClick={() => document.getElementById("px-in")?.click()}
          ...
          <div className="space-y-4">
            <div>
              <label className="mb-1 block text-sm text-zinc-400">
                {t("labels.blockSize")}: {blockSize}px
              </label>
              <input
                type="range"
                min={2}
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              >
                {processing ? t("status.processing") : t("buttons.pixelate")}
              </button>
              <button
